...
##  2015-03-29, AF: ported to rochePlot
##  2015-05-07, AF: ported to rochePlot
##  
...
if [[ ${#} -ne 2 && ${#} -ne 4 ]]; then
    ...
    echo "!> \file code_version.f90  Source file generated by CMake to report the rochePlot version used" > ${F90FILE}
    echo "" >> ${F90FILE}
    
    echo "!***********************************************************************************************************************************" >> ${F90FILE}
    echo "!> \brief  Report rochePlot version number" >> ${F90FILE}
    echo "" >> ${F90FILE}
    echo "module RP_version" >> ${F90FILE}
    ...
    echo "  " >> ${F90FILE}
    echo "  subroutine print_rochePlot_version(unit)" >> ${F90FILE}
    echo "    implicit none" >> ${F90FILE}
    echo "    integer, intent(in) :: unit" >> ${F90FILE}
    if [ ${RELEASE} == 'yes' ]; then
	echo "    character :: rochePlot_version*(99) = 'v"`cat .version`"'" >> ${F90FILE}
	echo "    character :: release_date*(99) = '"`date +"%F"`"'" >> ${F90FILE}
	
	echo "    " >> ${F90FILE}
	echo "    write(unit,'(A)', advance='no') 'rochePlot '//trim(rochePlot_version)//' ('//trim(release_date)//')'" >> ${F90FILE}
    else
	if [ -e .git/ ]; then  # Prefer revision number over release number
	    echo "    character :: rochePlot_version*(99) = 'rev."`git rev-list --abbrev-commit HEAD | wc -l`", hash "`git log --pretty="%h (%ad)" --date=short -n1`"'" >> ${F90FILE}
	#echo "  character :: code_version*(99) = 'rev."`git rev-list --abbrev-commit HEAD | wc -l`", "`git describe --tags`" "`git log --pretty="(%ad)" --date=short -n1`"'" >> ${F90FILE}  # Doesn't work on Mac OS(?)
	    ...
	    echo "    character :: rochePlot_version*(99) = 'revision "`bzr revno`"'" >> ${F90FILE}
	elif [ -e VERSION ]; then
	    echo "    character :: rochePlot_version*(99) = 'v"`grep 'Release version' VERSION | awk '{print $3}'`"'" >> ${F90FILE}
	elif [ -e doc/VERSION ]; then
	    echo "    character :: rochePlot_version*(99) = 'v"`grep 'Release version' doc/VERSION | awk '{print $3}'`"'" >> ${F90FILE}
	else
	    echo "    character :: rochePlot_version*(99) = '(unknown version)'" >> ${F90FILE}
	fi
	...
	echo "    " >> ${F90FILE}
	echo "    write(unit,'(A)', advance='no') 'rochePlot '//trim(rochePlot_version)//', compiled on '//trim(compile_date)//' with '// &" >> ${F90FILE}
	echo "         trim(compiler)//' '//trim(compiler_flags)" >> ${F90FILE}
    fi
    echo "    " >> ${F90FILE}
    echo "  end subroutine print_rochePlot_version" >> ${F90FILE}
    echo "  !*********************************************************************************************************************************" >> ${F90FILE}
    
  
    echo "  " >> ${F90FILE}
    echo "end module RP_version" >> ${F90FILE}
    echo "!***********************************************************************************************************************************" >> ${F90FILE}
    echo "" >> ${F90FILE}
